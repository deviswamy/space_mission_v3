...
| **State — UI** | Zustand |
| **Routing** | Wouter (with auth guards) |
...
Strict TypeScript is enforced across the entire codebase. Key rules:

- **No `as any` or unsafe casts** — use Zod `safeParse()` or type guards instead
- **Drizzle types are the source of truth** — infer all DB types from schema, never redeclare manually
- **Compose, don't duplicate** — use `Pick`, `Omit`, `&` to derive types from existing ones
- **Validate at boundaries** — all API inputs/outputs validated with Zod schemas

### Project Structure for Types

| Location | Purpose |
| --- | --- |
| `shared/schema/` | Drizzle table definitions + inferred types (`$inferSelect`, `$inferInsert`) |
| `shared/dtos.ts` | API response types — composed from schema types |
| `shared/contracts.ts` | Zod schemas for all API request/response contracts |
| `shared/branded-types.ts` | Type-safe IDs (`OrgId`, `UserId`, `MissionId`, etc.) |

### Drizzle Type Pattern

```typescript
// shared/schema/missions.ts
export const missions = pgTable('missions', { ... });

export type Mission = typeof missions.$inferSelect;      // full row
export type NewMission = typeof missions.$inferInsert;   // insert payload
export type MissionSummary = Pick<Mission, 'id' | 'title' | 'status' | 'startDate' | 'endDate'>;
```
...
export const CreateMissionSchema = z.object({
  ...
  skillRequirements: z.array(z.object({ skillId: z.string().uuid(), count: z.number().int().min(1) })),
});

// Backend handler — validate at entry point
...
-- Managed by better-auth (Drizzle adapter generates this)
-- Extended with our fields via additionalFields config
user
  id text PK
  name text
  email text UNIQUE
  email_verified boolean
  password_hash text           -- argon2 via better-auth
  org_id uuid FK → orgs.id    -- our extension
  role text                    -- 'director' | 'mission_lead' | 'crew_member'
  must_change_password boolean DEFAULT true
  created_at timestamptz
  updated_at timestamptz

-- Also managed by better-auth (do not write to directly)
...
> **Note:** The `user` table is generated by better-auth's Drizzle adapter. Never write raw SQL migrations for it — run `auth.generateSchema()` and let Drizzle diff. Our `additionalFields` (`orgId`, `role`, `mustChangePassword`) are included automatically. See [docs/plans/2026-03-10-auth-design.md](plans/2026-03-10-auth-design.md) for full configuration.
...
- better-auth schema generated via `auth.generateSchema()` and pushed with Drizzle
- `additionalFields` (`orgId`, `role`, `mustChangePassword`) present on better-auth's `user` table
- Our tables (`orgs`, `skills`, and all feature tables) defined with correct FK constraints
- `org_id` present on all tenant-scoped tables
- Enum types: `experience_level`, `mission_status`, `assignment_status`
- All migrations runnable with `bun run db:migrate`
